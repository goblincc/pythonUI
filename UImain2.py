...
class Window(third.Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def cal(self, datas, cur_sel):
        data = datas[self.data['维保阶段名称'] == cur_sel]
        ...
        data_cur_valid_visit = data[(data['回访时间'] <= dateEnd) & (data['回访时间'] >= dateStart) & (data['回访方式'] == '有效回访')]
        ...
        # ⑥当年累计有效回访量
        data_cur_valid_visit_year = data[(data['回访时间'] <= dateEndYear) & (data['回访时间'] >= dateStartYear) & (data['回访方式'] == '有效回访')]
        ...
        for group in cal_group:
            ...
            if cal_group_name == "项目分期":
                ...
                for pre in pre_group:
                    ...
                    # 1.当前回访量
                    data_cur_tmp = data_cur_visit[(data_cur_visit[cal_group_name] == group) & (data_cur_visit['项目'] == pre)]
                    ...
                    data_cur_valid_visit_year_tmp = data_cur_valid_visit_year[
                        (data_cur_valid_visit_year[cal_group_name] == group) & (data_cur_valid_visit_year['项目'] == pre)]
                    ...
                    data_cur_valid_visit_satisfy_year_tmp = data_cur_valid_visit_satisfy_year[
                        (data_cur_valid_visit_satisfy_year[cal_group_name] == group) & (data_cur_valid_visit_satisfy_year['项目'] == pre)]
                ...
                # 6.当年累计有效回访量
                data_cur_valid_visit_year_tmp = data_cur_valid_visit_year[data_cur_valid_visit_year[cal_group_name] == group]
                cur_valid_visit_year = data_cur_valid_visit_year_tmp['回访时间'].count()
                ...
                # 8.当年累计回访满意工单
                data_cur_valid_visit_satisfy_year_tmp = data_cur_valid_visit_satisfy_year[data_cur_valid_visit_satisfy_year[cal_group_name] == group]
