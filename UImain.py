...
class Window(fist.Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def calTableValue(self):
        ...
        data_response = data[(data["响应时间"] >= dateStart) & (data["响应时间"] <= dateEnd) & data["受理至响应间隔时长"] < 0.51]
        ...
        data_dur = data_time_notnull["业主关闭时间"].apply(lambda x: 0 if pd.isna(x) else self.date2Timestamp(x)) + \
                    data_time_notnull["非正常关闭时间"].apply(lambda x: 0 if pd.isna(x) else self.date2Timestamp(x)) + \
                    data_time_notnull["强制关闭时间"].apply(lambda x: 0 if pd.isna(x) else self.date2Timestamp(x)) - \
                    data_time_notnull["报事时间"].apply(lambda x: 0 if pd.isna(x) else self.date2Timestamp(x))
